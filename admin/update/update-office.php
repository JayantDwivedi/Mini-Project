...
<head>
    ...
    <title>Edit-Office-Record</title>
</head>

<body>
    <h1>Office Record Update</h1>
...
if (isset($_GET['b1'])) {
    ...
    if ($name != "" && $mobile != "" && $email != "" && $address != "" && $time != "" && $date != "" && $comments != "" && $area != "") {
        $query = "UPDATE `officeservice` SET `name` = '$name', `email` = '$email', `mobile` = '$mobile', `address` = '$address', `time` = '$time', `date` = '$date', `area` = '$area', `comment` = '$comments' WHERE `officeservice`.`mobile` = '$mobile';";
        $data = mysqli_query($connection, $query);
        if ($data) {
            echo "<p style='color:green;font-size:24px;text-align:center;'>Record Updated Succesfully <a href='http://localhost/Mini-Project/admin/office-admin.php'>UPDATED RECORD</a>";
        } else {
