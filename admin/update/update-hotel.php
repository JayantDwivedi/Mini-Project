    ...
    <form action="" method="GET">
        <table>
            ...
            <tr>
                ...
                <td> <input type="text" name="rooms" value="<?php echo $_GET['rms']; ?>"><br></br></td>
            </tr>
            <tr>
...
if (isset($_GET['b1'])) {
    ...
    if ($name != "" && $mobile != "" && $email != "" && $address != "" && $time != "" && $date != "" && $comments != "" && $rooms != "") {
        $query = "UPDATE `hotelservice` SET `name` = '$name', `email` = '$email', `mobile` = '$mobile', `address` = '$address', `time` = '$time', `date` = '$date', `rooms` = '$rooms', `comment` = '$comments' WHERE `hotelservice`.`mobile` = '$mobile';";
        $data = mysqli_query($connection, $query);
        if ($data) {
            echo "<p style='color:green;font-size:24px;text-align:center;'>Record Updated Succesfully <a href='http://localhost/Mini-Project/admin/hotel-admin.php'>RECORD UPDATED</a>";
        } else {
