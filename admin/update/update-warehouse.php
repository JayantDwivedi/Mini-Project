...
<head>
    ...
    <title>Edit-Warehouse-Record</title>
</head>

<body>
    <h1>Warehouse Record Update </h1>
    <form action="" method="GET">
        <table>
            ...
            <tr>
                <td> <label for="">Area <i class="fa fa-calendar" aria-hidden="true"></i></label></td>
                <td><input type="text" name="area" value="<?php echo $_GET['ar']; ?>"><br></br></td>
            </tr>
            <tr>
...
if (isset($_GET['b1'])) {
    ...
    if ($name != "" && $mobile != "" && $email != "" && $address != "" && $time != "" && $date != "" && $comments != "" && $area != "") {
        $query = "UPDATE `warehouseservice` SET `name` = '$name', `email` = '$email', `mobile` = '$mobile', `address` = '$address', `time` = '$time', `date` = '$date',`area` = '$area', `comment` = '$comments' WHERE `warehouseservice`.`mobile` = '$mobile';";
        $data = mysqli_query($connection, $query);
        if ($data) {
            echo "<p style='color:green;font-size:25px;text-align:center;'>Record Updated Succesfully <a href='http://localhost/Mini-Project/admin/warehouse-admin.php'>UPDATED RECORD</a>";
        } else {
