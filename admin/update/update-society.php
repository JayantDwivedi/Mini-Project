...
<head>
    ...
    <title>Edit-Society-Record</title>
</head>

<body>
    <h1>Society Record Update</h1>
    <form action="" method="GET">
        <table>
            ...
            <tr>
                <td> <label for="">Flats <i class="fa fa-calendar" aria-hidden="true"></i></label></td>
                <td> <input type="numbers" name="flats" value="<?php echo $_GET['fl']; ?>"><br></br></td>
            </tr>
            <tr>
...
if (isset($_GET['b1'])) {
    ...
    if ($name != "" && $mobile != "" && $email != "" && $address != "" && $time != "" && $date != "" && $comments != "" && $flat != "") {
        $query = "UPDATE `societyservice` SET `name` = '$name', `email` = '$email', `mobile` = '$mobile', `address` = '$address', `time` = '$time', `date` = '$date', `flats` = '$flat', `comment` = '$comments' WHERE `societyservice`.`mobile` = '$mobile';";
        $data = mysqli_query($connection, $query);
        if ($data) {
            echo "<p style='color:green;font-size:24px;text-align:center;'>Record Updated Succesfully <a href='http://localhost/Mini-Project/admin/society-admin.php'>UPDATED RECORD</a>";
        } else {
